...
def _parse_s6f11_report(full_text: str) -> dict:
    ...
    # Step 1: Extract all unsigned integers to find our key markers.
    ...
    # Step 2: Identify CEID and RPTID based on their known standard positions.
    try:
        ceid = uints[1]
        rptid = uints[2]
    ...
    # Step 4: CRITICAL FIX - Isolate the specific data payload.
    # We build a precise regex to find the RPTID tag and then capture the contents
    # of the list block that immediately follows it. This prevents contamination.
    # The `[\s\S]*?` is a non-greedy match that stops at the first closing `>`.
    ...
    # Step 5: Tokenize ONLY the isolated report body.
    values = re.findall(r"<(?:A|U\d)\s\[\d+\]\s(?:'([^']*)'|(\d+))>", report_body)
    flat_values = [s if s else i for s, i in values]
    
    # Step 6: Map these clean tokens to the field names from our config schema.
